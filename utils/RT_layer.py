...
class RT(nn.Module):
    ...
    def forward(self, node_features, edge_features_, return_edge=False):
        ...
        rel_rec, rel_send = self.init_adj(num_nodes, batch)                      # B x (N^2) x D
        edge_features = self.init_edge(node_features, rel_rec, rel_send)         # B x N x N x D=64
...
class RTTransformerLayer(nn.Module):
    def __init__(
        ...
        self.attention_layer = RTAttentionLayer(num_heads, node_dim, edge_dim, topk, threshold)
    ...
    def forward(self, node_features, edge_features):
        ...
        return node_features, edge_features # Node features:B x N x D=64; Edge features:B x N x N x D=64
...
class RTAttentionLayer(nn.Module):
    ...
    def forward(self, node_features, edge_features):
        ...
        # Apply sparse attention based on topk or threshold
        if self.topk is not None and self.topk < N:
            # Keep only top-k neighbors for each node
            topk_values, topk_indices = torch.topk(att_dist, self.topk, dim=-1)
            # Create a mask to zero out non-topk elements
            mask = torch.zeros_like(att_dist)
            mask.scatter_(-1, topk_indices, 1.0)
            att_dist = att_dist * mask
            # Renormalize to ensure probabilities sum to 1
            att_dist = att_dist / (att_dist.sum(dim=-1, keepdim=True) + 1e-8)
        
        if self.threshold is not None:
            # Keep only neighbors with attention weight above threshold
            mask = (att_dist > self.threshold).float()
            att_dist = att_dist * mask
            # Renormalize to ensure probabilities sum to 1
            att_dist = att_dist / (att_dist.sum(dim=-1, keepdim=True) + 1e-8)
...
# # 使用 topk，每个节点只关注最相关的5个邻居
# model = RT(topk=5)

# # 使用 threshold，只关注注意力权重超过0.1的邻居  
# model = RT(threshold=0.1)

# # 也可以同时使用两个条件
# model = RT(topk=10, threshold=0.05)